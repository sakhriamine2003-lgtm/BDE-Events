            ...
            <table class="min-w-full border border-gray-200">
                ...
                <thead class="bg-indigo-600 text-white">
                    <tr>
                        ...
                        <th class="py-3 px-4 border">Places Réservées</th>
                ...
                <tbody class="text-center">
                    @foreach ($Evenement as $ev)
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="border px-4 py-2 font-medium">{{ $ev->title }}</td>
                            <td class="border px-4 py-2">{{ $ev->date }}</td>
                            <td class="border px-4 py-2">{{ $ev->heure }}</td>
                            <td class="border px-4 py-2">{{ $ev->Lieu }}</td>
                            <td class="border px-4 py-2">{{ $ev->Prix }} DH</td>
                            <td class="border px-4 py-2">{{ $ev->maxPlaces }}</td>

                            <!-- Compte dynamique des réservations -->
                            <td class="border px-4 py-2">
                                {{ $ev->reservations ? $ev->reservations->count() : 0 }}
                            </td>
                            ...
                            <td class="border px-4 py-2 space-x-1">
                                @if (Auth::user()->role_user == 'Admin')
                                    ...
                                    <form action="{{ route('SupprimerEvent', $ev->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                        ...
                                        @csrf
                                    @elseif (Auth::user()->role_user == 'Etudiant')
